...
void llvm::cl::parser<TargetRegistryRef>::print(raw_ostream &os, const TargetRegistryRef &value) {
  if(value.isGlobal())
    os << "global";
  else
    os << "unknown";
...
bool llvm::cl::parser<TargetRegistryRef>::parse(Option &O, StringRef ArgName,
  ...
  if (Arg != "global")
    llvm::errs() << "Cannot parse target registery: " << Arg << ". Defaulting to global.\n";
