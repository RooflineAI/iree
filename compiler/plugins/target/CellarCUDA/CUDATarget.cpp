...
#include "CodeGen/CudaGPUCodeGenPipeline.h"
...
struct CUDAOptions {
  ...
  void bindOptions(OptionsBinder &binder) {
    static llvm::cl::OptionCategory category("Cellar CUDA HAL Target");

    binder.opt<std::string>(
        "iree-cellar-cuda-target", clTarget, llvm::cl::cat(category),
        llvm::cl::desc(
            // clang-format off
    ...
    binder.opt<std::string>(
        "iree-cellar-cuda-target-features", clTargetFeatures, llvm::cl::cat(category),
        llvm::cl::desc(
            "CUDA target features as expected by LLVM NVPTX backend; e.g. "
            "use '+ptxNN' to set PTX version to NN."));

    binder.opt<bool>(
        "iree-cellar-cuda-use-ptxas", clUsePtxas, llvm::cl::cat(category),
        llvm::cl::desc(
            ...
            "'--iree-cellar-cuda-use-ptxas-from'. To pass "
            "additional parameters to ptxas, use "
            "'--iree-cellar-cuda-use-ptxas-params', e.g. "
            "'--iree-cellar-cuda-use-ptxas-params=-v'"));

    binder.opt<std::string>(
        "iree-cellar-cuda-use-ptxas-from", clUsePtxasFrom, llvm::cl::cat(category),
        llvm::cl::desc("Uses the ptxas tool from the given path. Requires "
                       "'--iree-cellar-cuda-use-ptxas' to be true."));

    binder.opt<std::string>(
        "iree-cellar-cuda-use-ptxas-params", clUsePtxasParams, llvm::cl::cat(category),
        llvm::cl::desc("Passes the given additional parameters to ptxas. "
                       "Requires '--iree-cellar-cuda-use-ptxas' to be true."));
  }
...
static FailureOr<std::string> findPtxasCompiler(const CUDAOptions &options,
  ...
  *message = std::string(
      ...
      "--iree-cellar-cuda-use-ptxas-from=<path> flag");
  return failure();
}
...
public:
  ...
  getDefaultDeviceTarget(MLIRContext *context,
    ...
    targetRegistry.getTargetBackend("cellar-cuda")->getDefaultExecutableTargets(
...
public:
  ...
  std::string getLegacyDefaultDeviceID() const override { return "cellar-cuda"; }
  ...
  getExecutableTarget(MLIRContext *context) const {
    ...
    return b.getAttr<IREE::HAL::ExecutableTargetAttr>(
        /*backend*/b.getStringAttr("cellar-cuda"), b.getStringAttr("cuda-nvptx-fb"),
  ...
  buildConfigurationPassPipeline(IREE::HAL::ExecutableTargetAttr targetAttr,
    ...
    cellar::target::cuda::buildCudaGPUCodegenConfigurationPassPipeline(passManager);
  }
  ...
  void buildLinkingPassPipeline(OpPassManager &passManager) override {
    buildLLVMGPULinkingPassPipeline(passManager, /*backendTarget*/"cellar-cuda");
  }
...
struct CUDASession
  ...
  void populateHALTargetDevices(IREE::HAL::TargetDeviceList &targets) {
    // #hal.device.target<"cellar-cuda", ...
    targets.add("cellar-cuda",
                [&]() { return std::make_shared<CUDATargetDevice>(options); });
  }
  void populateHALTargetBackends(IREE::HAL::TargetBackendList &targets) {
    // #hal.executable.target<"cellar-cuda", ...
    targets.add("cellar-cuda", [&]() {
...
extern "C" bool iree_register_compiler_plugin_hal_target_cellar_cuda(
    mlir::iree_compiler::PluginRegistrar *registrar) {
  registrar->registerPlugin<cellar::HAL::CUDASession>(
      "hal_target_cellar_cuda");
  return true;
}

IREE_DEFINE_COMPILER_OPTION_FLAGS(cellar::HAL::CUDAOptions);
